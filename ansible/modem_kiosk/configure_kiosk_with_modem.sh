...
ansible-playbook -i "$dir_of_script"hosts.yml "$dir_of_script"create_and_dowload_ovpn.yaml  -e "fullnamekiosk=$full_name_ovpn_file" #Ансибл создает и забирает на локальный пк ovpn конфиг файл
...
ansible-playbook -i "$dir_of_script"inventory.ini -e "serial_kiosk=$serial_kiosk fullnamekiosk=$full_name_ovpn_file" "$dir_of_script"work_on_kiosk.yaml
